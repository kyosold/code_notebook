 ...
 */
static const void *body(S_MD5_CTX *ctx, const void *data, size_t size)
{
 ...
 *                  then the md5 digest is instead returned 
 *                  in raw binary format with a length of 16.
 * @param result Returns the hash as a 32-character hexadecimal number.
...
void s_md5(const char *str, size_t str_len, int raw_output, char *result, size_t result_size)
{
    ...
    {
        make_digest_ex(md5str, digest, 16);
        snprintf(result, result_size, "%s", md5str);
    }
}
 ...
 * @param raw_output When 1, returns the digest in 
...
{
    char res[1024] = {0};
    s_md5(argv[1], strlen(argv[1]), 0, res, sizeof(res));
    printf("%s\n", res);
}
#endif